"""Train XGBoost category classifier on clean split data."""
...
def _summary_path() -> Path:
    return config.METRICS_DIR / "xgboost_training_summary.json"


def _per_class_path() -> Path:
    return config.METRICS_DIR / "xgboost_per_class.csv"
...
def _run_optuna(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        model = XGBoostTicketClassifier(params=trial_params)
    ...
    config.XGBOOST_OPTUNA_SUMMARY_PATH.parent.mkdir(parents=True, exist_ok=True)
    config.XGBOOST_OPTUNA_SUMMARY_PATH.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    return summary
...
def train_xgboost_model(*, enable_optuna: bool | None = None) -> dict[str, Any]:
    ...
    model = XGBoostTicketClassifier(params=params, label_classes=class_names)
    model.fit(X_train, y_train, X_val=X_val, y_val=y_val)
    ...
    with mlflow_utils.start_run("train-xgboost-category"):
        ...
        if use_optuna and config.XGBOOST_OPTUNA_SUMMARY_PATH.exists():
            mlflow_utils.log_artifact(str(config.XGBOOST_OPTUNA_SUMMARY_PATH))
